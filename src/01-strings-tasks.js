...
function getRectangleString(width, height) {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      const index = getK(x, width) + 3 * getK(y, height);
      result += parts[index];
    }
  }
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < l; i += 1) {
    ...
    if (code >= 65 && code < 91) {
      result += String.fromCodePoint(min >= 65 ? min : max);
    } else if (code >= 97 && code < 123) {
      result += String.fromCodePoint(min >= 97 ? min : max);
    } else result += str[i];
  }
...
function getCardId(value) {
  ...
  const index = ranks.indexOf(rank) + ranks.length * suits.indexOf(suit);
  return index;
}
